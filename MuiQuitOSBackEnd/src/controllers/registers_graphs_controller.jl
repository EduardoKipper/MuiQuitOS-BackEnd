...
function generate_registers_graphs()
    ...
    Plots.theme(:ggplot2)
    green_palette = [:forestgreen, :seagreen, :limegreen, :darkgreen, :mediumseagreen, :olivedrab, :lawngreen, :springgreen, :mediumaquamarine, :palegreen]
    ...
    pie(labels, counts.count, title="Registers by Neighbourhood", color=green_palette[1:length(counts.count)])
    ...
    Plots.plot(hour_counts.hour, hour_counts.count, seriestype=:line, title="Registers by Time of Day", xlabel="Hour of Day", ylabel="Count", legend=false, color=green_palette[1])
    ...
    Plots.plot(string.(sorted_counts.month), sorted_counts.count, seriestype=:line, title="Registers by Month", xlabel="Month", ylabel="Count", legend=false, xrotation=60, color=green_palette[2])
    Plots.savefig(joinpath(CACHE_DIR, "registers_by_month_line.png"))
    println("Gráfico de registros por mês gerado.")

    # 4. Heatmap de registros por bairro e hora do dia (sem tema e sem paleta verde)
